...
def run_case(mode):
    with tempfile.TemporaryDirectory(prefix='telecom console ') as temp:
        ...
        try:
            ...
            launch_script='telecom_updater.py'
            if mode=='failure':
                # V71's existing snapshot_data returns Windows backslashes, while
                # its restore_data validator accepts only forward slashes. That
                # pre-existing launcher bug is unrelated to GUI detachment and
                # cannot be repaired by the three-file application update. Use
                # a valid backup reference for this readiness/rollback fixture;
                # normal/update/duplicate cases run the installed file unchanged.
                launch_script='failure_launcher.py'
                (home/launch_script).write_text("import sys\nimport telecom_updater as u\noriginal=u.snapshot_data\nu.snapshot_data=lambda *args:original(*args).replace(chr(92),'/')\nsys.exit(u.Launcher(u.ROOT).run())\n",encoding='utf-8')
            bat=home/'run.bat'
            bat.write_text('@echo off\ncd /d "%~dp0"\n"'+sys.executable+'" -u '+launch_script+'\nexit /b %errorlevel%\n')
            env=os.environ.copy()
            ...
            with (home/'cmd.log').open('wb') as log:
                ...
                assert cmd.wait(timeout=20)==0,(home/'cmd.log').read_text(encoding='utf-8',errors='replace')
